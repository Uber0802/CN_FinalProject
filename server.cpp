...
// handle_client function with log statements
    ...
    while (true) {
        ...
        if (bytes_read <= 0) {
            if (is_logged_in) {
                ...
                // No explicit logout message, but user might have closed the client.
            }
            close(client_socket);
            break;
        }
        ...
        if (!is_logged_in) {
            ...
            if (message == "1") {
                ...
                // LOG in desired format
                cout << current_user << " send message to " << target_user 
                     << " : " << user_msg << endl;
